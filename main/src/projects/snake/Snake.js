...
class Snake {
	...
	update(score, highscore, apple, gameState) {
		...
		if (this.tailLen > 2) {
			for (let i = 0; i < this.tailLen; i++) {
				...
				if (apple.x == this.tailX[i] && apple.y == this.tailY[i])
					apple.respawn();
			}
		}
	...
	draw(ctx) {
		...
		for (let i = 0; i < this.tailLen; i++) {
			ctx.fillRect(
				this.tailX[i] * this.width,
				50 + this.tailY[i] * this.width,
				this.width,
				this.width
			);
		}
...
export default function SnakeGame() {
	...
	return (
		<>
			{window.innerWidth >= 400 && window.innerHeight >= 425 ? (
				<div className="snake">
