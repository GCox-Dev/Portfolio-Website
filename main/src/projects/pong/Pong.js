...
class Ball {
	constructor() {
		...
		this.dx = Math.random() * 2 > 1 ? 5 : -5;
		this.dy = Math.random() * 2 > 1 ? 5 : -5;
	}

	draw(ctx, player1, player2) {
		if (
			(this.x + this.dx < player1.x &&
				this.y + this.dy + this.height > player1.y &&
				this.y + this.dy < player1.y + player1.height) ||
			(this.x + this.dx + this.width > player2.x &&
				this.y + this.dy + this.height > player2.y &&
				this.y + this.dy < player2.y + player2.height)
		) {
			this.dx *= -1.1;
			this.dy *= 1.1;
		}
		...
		<>
			{window.innerWidth >= 800 && window.innerHeight >= 600 ? (
				...
				<div className="noresult">
					<h4>Resolution Not Supported</h4>
				</div>
			)}
			<a href="../">
