...
export default function GameOfLife() {
	...
	useEffect(() => {
		...
		setInterval(() => {
			if (mouse.click && !mouse.ctrl) {
				let offsetX =
					window.innerWidth / 2 - (WIDTH * camera.zoom) / 2 + camera.offX;
				let offsetY =
					window.innerHeight / 2 - (HEIGHT * camera.zoom) / 2 + camera.offY;
				if (
					mouse.x > offsetX &&
					mouse.x < offsetX + WIDTH * camera.zoom &&
					mouse.y > offsetY &&
					mouse.y < offsetY + HEIGHT * camera.zoom
				) {
