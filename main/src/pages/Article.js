...
export function MapGenerationArticle() {
    return (
        <Article cur={data[7]}>
            ...
            <p>Perlin Noise was developed by Ken Perlin in the early 80s. It is an extremely useful and versatile algorithm and can be used to generate fire effects, water, clouds, and in our case terrain. For a more in depth explanation visit this extremely helpful <a href="https://adrianb.io/2014/08/09/perlinnoise.html">article</a></p>
            <Image src={"/images/map-generation/noise2.png"} />
            <Subtitle>Diamond Sqaure Algorithm</Subtitle>
            <p>The diamond square algorithm is another way to generate height maps. However instead of finding random values for every point within the area random values are assigned to the corners of the height map. The first step is the diamond step. For every square in the array we take the midpoint of that square and set it to the average of the four corners. The second step is the square step. For every diamond in the array we take the midpoint of that diamond and set it the average of the four corners plus a random value. These steps are repeated, each time with smaller increments until the array is filled with values.</p>
